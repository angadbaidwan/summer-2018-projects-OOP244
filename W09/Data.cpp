...
namespace sict {
	...
	void answers(const int* year, const int* population, const int* violentCrime,
		...
		std::cout << "Population change from " << year[0] << " to "
			<< year[n - 1] << " is " << static_cast<double>(population[n - 1] - population[0]) / 1000000
			<< " million" << std::endl;
		...
		else if (violentCrime[n - 1] < violentCrime[0]) {
			std::cout << "Violent Crime trend is down" << std::endl;
		}
		else if (violentCrime[n - 1] == violentCrime[0]) {
			std::cout << "Violent Crime is neither up nor down" << std::endl;
		}
		
		// Q3 print the GTA number accurate to 0 decimal places
		std::cout << "There are " << (average(grandTheftAuto, n) / 1000000)
